...
function fillTableBody(tbody, config, data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    editButton.onclick = () => showEditModal(config, data[i]);

    buttonsContainer.append(deleteButton, editButton);

    actionCell.append(buttonsContainer);
...
function showAddModal(config) {
  ...
  handleFormSubmit(form, modal, config, null); // Pass null for itemId when adding
}

function showEditModal(config, item) {
  const modal = createModal();
  const modalContent = createModalContent(modal);
  const form = createForm(config);
  ...
  form.querySelectorAll("input, select").forEach((input) => {
    ...
    if (value !== undefined) {
      if (input.type === "date" && input.name === "birthday") {
        // Date for input
  ...
  handleModalClose(modal, form);
  handleFormSubmit(form, modal, config, item.id); // Pass item.id for editing
}
...
function createForm(config) {
  ...
  buttonsContainer.append(createSubmitButton());
...
function handleFormSubmit(form, modal, config, itemId) {
  ...
  inputs.forEach((input) => {
    ...
    addEnterKeyListener(input, form);
  });
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      if (itemId) {
        await editItem(config.apiUrl, itemId, newItem);
        alert("Дані оновлено");
      } else {
...
function getColorLabel(color) {
  const colorLable = document.createElement("color-label");
  colorLable.className = "color-label";
  colorLable.style.backgroundColor = color;
  colorLable.title = color;
  return colorLable.outerHTML;
}

async function editItem(apiUrl, id, item) {
  try {
    const processedItem = { ...item };
    if (processedItem.price) {
      processedItem.price = parseFloat(processedItem.price) || 0;
