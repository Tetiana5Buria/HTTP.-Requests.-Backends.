function DataTable(config) {
  ...
  function fillTableBody(tbody, config, data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      actionCell.append(deleteButton, editButton); // Reordered: Delete first, then Edit
  ...
  function handleFormSubmit(form, modal, config, itemId) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        if (itemId) {
          await updateItem(config.apiUrl, itemId, newItem);
          alert("Дані оновлено");
        } else {
        ...
        alert("Помилка при " + (itemId ? "оновленні" : "додаванні") + ": " + error.message);
      }
    });
  }
  ...
  async function updateItem(apiUrl, id, item) {
    try {
      const processedItem = { ...item };
      if (processedItem.price) {
        processedItem.price = parseFloat(processedItem.price) || 0;
